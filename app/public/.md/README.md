...
```
inmopress/
├── inmopress-core/          (Base: CPTs, taxonomías, ACF, roles, Activity Log, Matching, Automation, AI, Performance)
├── inmopress-frontend/      (Panel agente, shortcodes, KPIs, Charts)
├── inmopress-blocks/        (16 Bloques Gutenberg)
├── inmopress-licensing/     (Sistema licencias + Stripe completo)
├── inmopress-emails/        (SMTP + IMAP, Templates, Queue)
├── inmopress-api/           (API REST con JWT, Rate Limiting)
└── inmopress-printables/    (Generación PDFs con mPDF)
```
